...
public class OrderService {
   ...
   /*public ClientSession createSession(ClientSession session, PrintWriter out) {
    	session = new ClientSession(out);
    	return session;
    }
    public void SetCart() {
    	//db에 출력된 메뉴들을 클라가 클릭함
    }
    public List GetCart() {
    	return 
    } */
    ...
    public void handleNewOrder(String msg, ClientSession session, PrintWriter out) {
        ...
        session.setNo(dto.getNo()); session.setName(dto.getName()); session.setStatus(dto.getStatus());
        //3. 손님 대기열에 추가
